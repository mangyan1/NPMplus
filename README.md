...
- live ban view in the admin UI ("CrowdSec" page, admin only): current crowdsec decisions incl. the anubis honeypot bans, refreshed every 10s, fully local via a read-only LAPI bouncer key - no third-party console. Needs `/data/crowdsec/lapi-ui.key` inside the container; setup-npmplus.sh creates it when crowdsec is enabled (also on `--update`). Manual, on installs made before this existed: `docker exec crowdsec cscli bouncers add npmplus-ui -o raw | sudo tee /opt/npmplus/crowdsec/lapi-ui.key >/dev/null && sudo chmod 600 /opt/npmplus/crowdsec/lapi-ui.key`
