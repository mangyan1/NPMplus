...
- Pointed the smoke-test installer self-check at the commit under test. The setup script compares itself against the published develop copy before update runs, so a PR that changes the script without bumping `SCRIPT_VERSION` always failed the update steps with "different content with the same SCRIPT_VERSION" - the guard was comparing the PR tree against develop by design. The workflow now rewrites `SELF_URL` to the checked-out commit, so the stale-guard still runs (and still refuses a genuinely stale script) while PR-side script changes are exercised end to end.
