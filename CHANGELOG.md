...
- Rejected the upstream dependency bumps that arrived younger than the pnpm `minimumReleaseAge` supply-chain window (backend `json-schema-ref-parser` 16.0.2, `mysql2` 3.24.4, `swagger-ui-dist` 5.32.15, `undici` 8.10.2; frontend `rolldown` 1.2.7 bindings, `postcss` 8.5.28, `react-aria` 3.52.1). The lockfiles were restored to the reviewed versions that satisfy the policy; the renovate/dependency-updates cron will pick each package up once it ages past the window. This unblocks lint-and-format, the image build, and boot-resilience for the upstream merge.
