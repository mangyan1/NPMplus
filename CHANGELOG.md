...
- Reconciled upstream develop through `a1ac84c8` (real merge commit, never squashed — [PR #29](https://github.com/mangyan1/NPMplus/pull/29), landed as `bfa66942`). Adopted upstream's per-row second-factor model (`7efc56c0`) and its migration: each factor now lives in its own `auth` row under a `type` column, and the migration clears `meta` on password rows. The fork's `auth.meta` factors could not be kept — that migration would have wiped them, silently disabling 2FA and locking out enrolled users — so the fork's protections were re-expressed on the row model: TOTP replay is still an id-scoped atomic claim of the matched timestep, the enrollment code's own step is stamped when the pending row is promoted, and backup codes stay single-use through a row-existence-guarded delete. Existing enrollments migrate without resetting their secrets. Dependency bumps taken: none — upstream `6887f082` and `20b56ee8` published inside the 7-day `minimumReleaseAge` window, so both manifests and lockfiles stay on the reviewed versions and the renovate cron ages them in. `validateAccessLists` now takes upstream's `(access, proxyHost)` argument order with the fork's stricter non-admin ownership scoping unchanged (this supersedes the order recorded for rc.8); access-list hashing takes upstream's async bcrypt at the fork's cost 6; the DB-backed token challenge sessions are kept instead of upstream's in-memory `consumedChallenges` map, which its own message concedes does not survive a restart. Also included: alpine 3.24.2 in both Dockerfiles, uppercase-hex IPv6 bindings, a stricter certificate domain-name pattern, and `ubuntu-26.04-arm` for the aarch64 build. Verified by 11/11 PR checks — including the upstream sync contracts gate that had been fail-closed red — plus 163 backend and 15 frontend tests, and a green post-merge `develop` push covering boot-resilience and the caddy image scan.

- Landed the CrowdSec dashboard on the Overview tab again; the Attackers investigation view stays one click away in the tab bar. This supersedes the rc.8 note below that described Attackers as the default view.

- Fixed the CrowdSec symlink trap that kept detection signatures from ever refreshing on the read-only root filesystem. The image entrypoint bridges its preloaded datafiles (the GeoLite2 databases, the Cloudflare IP lists, `detect.yaml`, the SEO bot lists, and the `trace` directory) into `/var/lib/crowdsec/data` as absolute symlinks into `/staging`, and `read_only: true` makes that target unwritable, so `cscli hub upgrade` followed the link into an EROFS and aborted the whole upgrade — one warning line, silenced by the entrypoint's own `|| true`, with no signature ever refreshed. Fresh installs now seed the staged datafiles into `/data` as real files while the container is still down: only what is missing is filled, and only symlinks that point into `/staging` are replaced, so an operator's edited `detect.yaml` and files already upgraded from the hub survive a reinstall, and the staged `trace` directory is materialized as a real directory on both the install and the update path (the seed and the wrapper's dereference both skipped directories, leaving the same trap in place for anything writing into it). The container-side dereference runs before the hub refresh on every start and is pinned by security invariant rule 8, so the line cannot be dropped as a clean change. The hub refresh is advisory again: the unguarded `cscli hub update` aborted the safe-update wrapper right after a successful update — an offline box turned a working update into a failed one — and only the upgrade reports failures now, with the smoke test failing the update leg when the log reports hub upgrade failures (the guard was an errexit-exempt `! grep` that could not fail the step no matter what the log said). Every mutating seed command now exits loudly on failure, and the installer's remedy message points at the `--update` path that repairs an unseeded directory. Verified against the pinned image: a fresh install starts with real datafiles, an operator symlink pointing outside `/staging` survives a reinstall, and a blocked write fails the seed instead of returning success.

See the [release notes](.github/release-notes/v2.15.1-mangyan1.rc.9.md) for installation and validation guidance.
...
- Reassessed all 24 container CVE exceptions against current upstream image scans and corrected their component/fixed-version explanations. All remain present; the 2026-10-04 expiry dates are unchanged.
