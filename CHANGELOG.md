...
- Existing installs migrate off the fail-open bouncer posture automatically: the daily heal script's new step 5 rewrites a seeded `MODE=live` to `MODE=stream` and, when AppSec is wired, `APPSEC_FAILURE_ACTION=passthrough` to `deny`, then restarts npmplus so the change takes effect. Operators who want the old behavior opt out once with `touch /data/crowdsec/keep-fail-open`; `crowdsec-doctor.sh` step 8d points at the marker. The migration is exercised by `tests/heal-migration.py` in CI (four fixture cases: migrate-and-restart, marker opt-out, unwired AppSec keeps only the mode change, already-fail-closed confs untouched).
