...
- the parser metric card understands CrowdSec 1.8's rename of `cs_parser_hits_*` to `cs_node_hits_*` (new names preferred, old names as fallback); a permanently empty parser table is normal for stacks fed only through AppSec — no log lines are parsed there, so the card shows its "no parser activity" note rather than being broken
