...
- anubis: image and policy are pinned to the same release, status codes adjusted for auth_request, bbolt store instead of memory, persistent directory ownership matched to the image's non-root user, honeypot IP logging, catch-all that challenges everything unmatched (the install prompt defaults to yes now; anubis-protected hosts serving APIs/RSS/uptime monitors should answer no)
