...
- UFW firewall: default deny, the detected/configured SSH port plus 443 (+80 or 81 if you opt in); ssh can be restricted to a subnet (e.g. 192.168.1.0/24), and any pre-existing rule set — including inactive, DENY-only or LIMIT-only configurations — is preserved unless you explicitly approve a reset
- anubis: image and policy are pinned to the same release, status codes adjusted for auth_request, bbolt store instead of memory, persistent directory ownership matched to the image's non-root user, honeypot IP logging, optional catch-all that challenges everything unmatched (kept off by default for API, webhook, and licensing compatibility; answer yes at the prompt or use `--update --enable-anubis-catchall` to opt in)
- honeypot to crowdsec bridge: IPs caught in anubis' honeypot are auto-banned for 7 days via a 5 minute cron; `--update --enable-anubis-catchall` turns the catch-all on for an existing install
- integrated security view in the admin UI ("CrowdSec" page, admin only): current decisions and Anubis honeypot bans, an attack-mix donut grouped by attack type with a per-interval activity strip, geolocation, count-aware scenario/country/ASN/IP/target filters, bounded server-side alert-history pagination, a dedicated AppSec WAF tab with configuration state, inspected/passed/blocked traffic, and the top triggered rules with hit counts, a WAF verdict card and bouncer decision-hit counts on the overview (bans actually served to the proxy, not just collected), CrowdSec Prometheus summaries, and opt-in browser alerts for attack spikes, active-ban changes, or LAPI outages while the page is open. It queries the existing CrowdSec LAPI directly and creates no second dashboard service or database. Unban (deletes the exact decision and lands in the audit log), manual ban, historical alert details, and per-ban context use a LAPI machine account because bouncer keys are read-only. Needs `/data/crowdsec/lapi-ui.key` and `/data/crowdsec/lapi-ui-machine.key`; `setup-npmplus.sh` creates and heals both. Prometheus is read on the private Docker network at `http://crowdsec:6060/metrics` and is published only to host loopback for host-network compatibility; metrics failure degrades only that panel.
- attack-map meteors fly from each attacker origin toward the instance, marked by a green "your instance" dot: the instance location comes from the `HOME_LATITUDE`/`HOME_LONGITUDE` env pair when set, otherwise the instance's public IP is geolocated once through ipwho.is (only the IP leaves the box, nothing about your traffic), cached for 24h with a 1h failure backoff; without a location the map degrades to marking origins only. Meteors are illustrative, not network routes
- the parser metric card understands CrowdSec 1.8's rename of `cs_parser_hits_*` to `cs_node_hits_*` (old names preferred, new names as fallback); a permanently empty parser table is normal for stacks fed only through AppSec — no log lines are parsed there, so the card shows its "no parser activity" note rather than being broken
- proxy host status dot reflects backend reachability, not just config validity: saving a proxy host tcp-probes its forward destination (3 second timeout, skipped for path/empty schemes, unix sockets, and upstream names) and the dot shows green = reachable, orange = unreachable (hover for the connect error), yellow = disabled, red = nginx config invalid (hover for the nginx error). The probe runs at save time, so a backend that dies later keeps its last state until the next edit. Other host types keep the config-validity dot.
...
11. Gravatar avatars are downloaded server-side and served locally. Users seeded by older versions without one (the installer's initial admin) get their avatar backfilled automatically on their next login
12. Maybe setup crowdsec (see below)
13. Please report all (migration) issues you may have

# Crowdsec

For upgrade behavior, session renewal, Argon2 password migration,
...
- By default, the container will run your `cron.sh` script and reload nginx on container start and then on fixed clock hours (default every hour, `0 */N * * *`). You can change this interval by setting the `ECH_ROTATION_INTERVAL` environment variable in your `compose.yaml`; the interval is measured against the clock, not the container start time.
...
2. If you enable `LOGROTATE` the access and error (also level “warn”), logs will be written to your disk and rotated daily with three rotations kept (a cron checks the rotation hourly). The access logs use these formats: [http](https://github.com/ZoeyVid/NPMplus/blob/c6a2df722390eb3f4377c603e16587fe8c74e54f/rootfs/usr/local/nginx/conf/nginx.conf#L30) and [stream](https://github.com/ZoeyVid/NPMplus/blob/c6a2df722390eb3f4377c603e16587fe8c74e54f/rootfs/usr/local/nginx/conf/nginx.conf#L249). These include user information (like IPs), so make sure to also mention that these exist and what you are doing with them.
